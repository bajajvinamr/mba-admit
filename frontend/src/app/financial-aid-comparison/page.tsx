...
export default function FinancialAidComparisonPage() {
 ...
 {packages.length > 2 && (
 <button onClick={() => removePackage(p.id)} className="text-muted-foreground hover:text-red-400"><Trash2 size={14} /></button>
 )}
 </div>

 <div className="flex flex-wrap gap-1 mb-3">
 {Object.keys(PRESETS).map((key) => (
 <button key={key} onClick={() => applyPreset(p.id, key)} className="text-[9px] px-2 py-0.5 bg-foreground/5 hover:bg-foreground/10 rounded-full text-muted-foreground">{key}</button>
 ))}
 </div>
 ...
 <div className="flex justify-between text-xs"><span className="text-muted-foreground">Total Cost (2yr)</span><span className="font-bold text-foreground">{fmt(c.totalCost)}</span></div>
 <div className="flex justify-between text-xs"><span className="text-muted-foreground">Total Aid (2yr)</span><span className="font-bold text-emerald-600">-{fmt(c.totalAid)}</span></div>
 <div className="flex justify-between text-xs border-t border-border/5 pt-2"><span className="text-muted-foreground font-medium">Net Cost</span><span className="font-bold text-foreground text-sm">{fmt(c.netCost)}</span></div>
 <div className="flex justify-between text-xs"><span className="text-muted-foreground">Debt at grad</span><span className="text-muted-foreground">{fmt(c.totalDebt)}</span></div>
 <div className="flex justify-between text-xs"><span className="text-muted-foreground">Payback period</span><span className="text-muted-foreground">{c.paybackYears} years</span></div>
 </div>
 </div>
 );
 })}
 ...
 {packages.length < 5 && (
 <button onClick={addPackage} className="editorial-card p-5 flex items-center justify-center gap-2 text-muted-foreground hover:text-muted-foreground hover:border-border/20 border-2 border-dashed border-border/10 transition-colors min-h-[200px]">
 ...
 <h2 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-4">Comparison Summary</h2>
 <div className="overflow-x-auto">
 <table className="w-full text-xs">
 <thead>
 <tr className="border-b border-border/10">
 <th className="text-left py-2 px-3 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Metric</th>
 {packages.map((p) => <th key={p.id} className="text-right py-2 px-3 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">{p.school}</th>)}
 </tr>
 </thead>
 <tbody>
 {[
